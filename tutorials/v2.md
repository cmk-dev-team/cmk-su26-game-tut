...
```package
cmk-su26-game-tut=github:cmk-dev-team/cmk-su26-game-tut#v1.0.59
```
...
```template
PlayerBlocks.onRemainingTime(0, function () {
    GameSettings.endGame()
    HunterSettings.removeHunter(HunterRemoveTarget.HunterA)
})
PlayerBlocks.onChatCommand("go", function () {
    VariableBlocks.setBountyStart(0)
    VariableBlocks.setTimeLimit(60)
    VariableBlocks.setLives(1)
    ...
    HunterSettings.resumeHunters(HunterRemoveTarget.HunterA)
})
```
