...
ミッション番号は1～3から選びます。
「ボタンを おした とき」または「ミッションの せいげん じかんが きた とき」のイベント内で、
「ミッションの せいこうを ◯ に おしらせする」「ミッションの しっぱいを ◯ に おしらせする」ブロックを使って
結果を決めます。
条件で結果を分けるときは、MakeCode標準の「もし」ブロックを使います。
デバッグ用チュートリアルでは、条件に「オークの ボタンが おされていた」が最初から入ります。
「ボタンを おした とき」はミッションの開始前でも使えます。
「〇〇の ボタンの じょうたいを リセットする」で、記録した押下状態を戻せます。
`v1.0.37` では旧形式のミッション設定コードを互換用として保持し、
新しく配置するブロックにはボタン種類を選べる新形式を使用します。

ゲーム開始は「ゲームを かいしする」ブロックで行います。
ブロックの「＋」を押すと、開始時のカウントダウン秒数を追加できます。
